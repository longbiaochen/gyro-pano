...
deconcept.SWFObject.prototype = {
	...
	getSWFHTML : function() {
		...
		if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
			...
			b = this.getVariablePairs().join("&");
			if (b.length > 0)
				a += '<param name="flashvars" value="' + b + '" />';
			a += "</object>"
		}
		return a
	},
					...
					if ( b = h[b.id])
						if (b == a.target) {
							try {
								if (b.jswheel)
									b.jswheel(c);
								else if (b.externalMouseEvent)
									b.externalMouseEvent(c);
								else if (b.enable_mousewheel_js_bugfix) {
...
function createkrpanoJSviewer(c, a, b, e, d, i, l) {
	function m() {
		...
		if ( typeof krpanoJS === "undefined")
			h = l ? "ERROR:<br/><br/>Adobe Flashplayer 10 or higher needed!<br/><br/><br/><br/>" : "ERROR:<br/><br/>HTML5 Version not available!<br/><br/><br/><br/>";
		else if (krpanojs_init(f) == false)
			h = "LICENSE ERROR";
		if (h)
			document.getElementById(f.htmltarget).innerHTML = '<table width="100%" height="100%"><tr valign="middle"><td><center>' + h + "</center></td></tr></table>"
	}
	...
	if (d === undefined || d == true) {
		if (!window.krpanoreg || !window.krpanokey) {
			document.write('<script src="../krpanoiphone.license.js" type="text/javascript" charset="UTF-8"><\/script>');
			j = true
		}
		if ( typeof krpanoJS === "undefined") {
			document.write('<script src="../krpanoiphone.js" type="text/javascript"><\/script>');
			j = true
		}
	}
	var f = {};
...
function createkrpanoSWFviewer(c, a, b, e, d, i, l) {
	...
	if (j.indexOf("chrome") >= 0 || j.indexOf("ipad") >= 0 || j.indexOf("iphone") >= 0 || j.indexOf("ipod") >= 0)
		f = true;
	else {
	...
	if (h >= 9) {
		for (; ; )
			if (document.getElementById(a))
				a += String.fromCharCode(48 + Math.floor(9 * Math.random()));
			else
				break;
		...
		n = 'Adobe Flashplayer 10 or higher needed<br/><br/><br/><a href="http://www.adobe.com/go/getflashplayer/" target="_blank" style="text-decoration:none;"><small>...click here to download...</small></a><br/><br/>';
	g = {};
...
function embedpano(c) {
	...
	if (d) {
		...
		if (c.vars)
			for (var j in c.vars) typeof c.vars[j] != "function" && a.addVariable(j, c.vars[j]);
		a.embed(d)
	} else
		alert("ERROR: embedpano() - target needed")
}
...
function createPanoViewer(c) {
	...
	a.isHTML5possible = function() {
		...
		if (d.indexOf("ipad") >= 0 || d.indexOf("iphone") >= 0 || d.indexOf("ipod") >= 0)
			return true;
	...
	a.setSWFPath = function(b) {
		a.pswfpath = b;
		if (a.pxml == "krpano.swf")
			a.pxml = a.pswfpath.split(".swf").join(".xml")
	};
	...
	a.passQueryParameters = function() {
		...
		if (b) {
			...
			for (var e = 0; e < b.length; e++) {
				...
				d = d.substring(i + 1);
				if (l == "useHTML5" || l.toLowerCase() == "html5")
					a.HTML5 = d;
				else
					a.addVariable(l, d)
			}
	...
	a.embed = function(b) {
		...
		if (a.ptarget) {
			...
			if (b == "always" || b == "force" || (b == "whenpossible" || b == "prefer") && a.isHTML5possible()) {
				...
				if (e >= 0)
					b = a.pswfpath.slice(0, e + 1);
				e = true;
				if (c.html5licenseload === false)
					e = false;
				b = createkrpanoJSviewer(a.pid, a.pwidth, a.pheight, b, e, a.bgcolor)
			} else
				b = createkrpanoSWFviewer(a.pswfpath, a.pid, a.pwidth, a.pheight, a.bgcolor, b != "never" && b != "no" && b != "false", c);
			...
			alert("ERROR: createPanoViewer.embed() - target needed")
	};
	c.wmode && a.addParam("wmode", c.wmode);
	c.passQueryParameters && a.passQueryParameters();
	return a
